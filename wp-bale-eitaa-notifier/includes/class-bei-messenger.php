...
final class Bei_Messenger {
	...
	private function friendly_error( $desc ) {
		...
		$map = array(
			...
			// راهنماهای اختصاصی دامنه (قبل از خطاهای شبکهٔ عمومی):
			'api.callmebot.com' => __( 'سرور شما به api.callmebot.com نمی‌رسد — این دامنه را هم مثل api.telegram.org در فهرست مجاز «مسدودکنندهٔ درخواست‌های خارجی» و فایروال قرار دهید؛ یا در کارت واتساپ «آدرس API (سفارشی/رله)» را روی دامنهٔ اختصاصی تنظیم کنید.', 'bale-eitaa-notifier' ),
